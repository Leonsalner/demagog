...
ALTER TABLE clanky ALTER COLUMN embedding TYPE vector(${ARTICLE_EMBEDDING_DIMENSIONS}) USING NULL::vector(${ARTICLE_EMBEDDING_DIMENSIONS});
...
async function createIndex(supabase: SupabaseClientAny): Promise<void> {
  ...
  if (error) {
    ...
    throw new Error(
      `Failed to create the ${ARTICLE_EMBEDDING_DIMENSIONS}d HNSW index automatically: ${formatRpcError(error as RpcError)}\nRun the SQL from scripts/setup-supabase.sql manually in the Supabase SQL editor.`,
    );
  }
}
