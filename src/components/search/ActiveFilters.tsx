  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-2">
      {activeChips.map((chip) => {
        let chipClass =
          "inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1 text-sm font-medium text-slate-700 shadow-sm dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200";

        if (chip.isVerdict && VERDICT_THEME[chip.value as Verdict]) {
          chipClass = `inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-sm font-medium shadow-sm ${VERDICT_THEME[chip.value as Verdict].chipActive}`;
        }

        return (
          <span key={`${chip.key}-${chip.value}`} className={chipClass}>
            {chip.label}
            <button
              type="button"
              onClick={() => removeFilter(chip.key, chip.value)}
              className="ml-1 inline-flex h-4 w-4 items-center justify-center rounded-full opacity-60 hover:bg-black/10 hover:opacity-100 dark:hover:bg-white/20"
